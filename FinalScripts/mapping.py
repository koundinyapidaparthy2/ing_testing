...
def merge_jsons(books_with_images_path, output_data_path, merged_output_path):
    ...
    for ean, output_info in output_data.items():
        if ean in books_data:
            ...
            original_price = float(output_info.get('price', 0))  # assuming price is a number, default is 0 if missing
            discount_level = float(output_info.get('discount_level', 0))  # assuming discount is in percentage

            # Calculate sale price by applying the discount
            saleprice = original_price - (original_price * (discount_level / 100))
            
            books_data[ean]['saleprice'] = round(saleprice, 2)  # round to 2 decimal places for currency format
